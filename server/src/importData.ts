...
import { moods, genreMap } from './routes/suggestion';
...
const musicMoodMap: { [key: string]: string } = {
  'neutral': 'sad',
  'energetic': 'happy',
  'melancholic': 'sad',
  'aggressive': 'angry',
...
function deriveMoodFromGenres(genres: string[], type: keyof typeof genreMap): string | null {
  console.log(`[DeriveMood] ${type} için genre'lar işleniyor: ${JSON.stringify(genres)}`);

  if (!Array.isArray(genres) || genres.length === 0 || !genreMap[type]) {
  ...
  const moodMatches: { [key: string]: number } = {};
  const relevantGenreMap = genreMap[type];

  for (const genre of genres) {
    const normalizedGenre = genre.toLowerCase().trim();
    console.log(`🔍 [DeriveMood] Genre işleniyor: "${normalizedGenre}"`);
    
    for (const [turkishMood, englishMood] of Object.entries(moods)) {
      const correspondingGenres = relevantGenreMap[turkishMood as keyof typeof relevantGenreMap];
...
function mapContent(item: any, type: string) {
  let assignedMood: string | null = null;
  let title = '';
  let description = '';
  let imageUrl = '';
...
async function importAll() {
  try {
    ...
    let movies = [];
    try { movies = JSON.parse(fs.readFileSync(path.join(dataDir, 'movies.json'), 'utf-8')); } catch (e) { console.error('❌ movies.json okunamadı:', e); }
    let books = [];
    try { books = JSON.parse(fs.readFileSync(path.join(dataDir, 'books.json'), 'utf-8')); } catch (e) { console.error('❌ books.json okunamadı:', e); }
    let series = [];
    try { series = JSON.parse(fs.readFileSync(path.join(dataDir, 'series.json'), 'utf-8')); } catch (e) { console.error('❌ series.json okunamadı:', e); }
    let music = [];
    try { music = JSON.parse(fs.readFileSync(path.join(dataDir, 'music.json'), 'utf-8')); } catch (e) { console.error('❌ music.json okunamadı:', e); }
    ...
    const allContent = [
      ...movies.map((item: any) => {
        ...
        if (!mappedItem) {
          const skippedMovie: { type: string, title: string, genres?: string[], mood: string | null } = { type: 'movie', title: item.title || item.Title || '', genres: item.genres, mood: null };
      ...
      ...books.map((item: any) => {
        ...
        if (!mappedItem) {
          const skippedBook: { type: string, title: string, genres?: string[], mood: string | null, moodTags?: string[] } = { type: 'book', title: item.title || item.Title || item.name || '', genres: item.genres, moodTags: item.moodTags, mood: null };
      ...
      ...series.map((item: any) => {
        ...
        if (!mappedItem) {
          const skippedSeries: { type: string, title: string, genres?: string[], mood: string | null } = { type: 'series', title: item.Title || item.title || '', genres: item.Genre, mood: null };
      ...
      ...music.map((item: any) => {
        ...
        if (!mappedItem) {
           ...
           const skippedMusic: { type: string, title: string, genres?: string[], mood: string | null, moodTags?: string[] } = { type: 'music', title: item.title || item.Title || item.name || '', moodTags: undefined, mood: null };
...
--- Atlanan Öğeler Özeti (${skippedItems.length} adet) ---`);
      skippedItems.forEach((item: { mood: string | null, type: string, title: string, genres?: string[], moodTags?: string[] }) => {
...
`);
    ...
    for (const type of contentTypes) {
      ...
      for (const mood of moodCategories) {
        ...
        const count = await Content.countDocuments({ type: type, mood: mood });
